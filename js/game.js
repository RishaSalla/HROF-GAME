...
let gameActive = true;
let currentTurn = 'red'; 
let currentMemberIndex = { red: 0, purple: 0 }; // لتتبع دور العضو
let isMuted = false;
...
function switchScreen(id) {
    ...
    const t = document.getElementById(id); if(t){ t.style.display='flex'; setTimeout(()=>t.classList.add('active'),50); }
}
...
function startGame() {
    ...
    if (gameSettings.teams === 'individual') {
        ...
        // تجميع أعضاء الفريق من الخانات المضافة
        const m1 = Array.from(document.getElementById('team-1-members-list').querySelectorAll('input')).map(i=>i.value).filter(v=>v);
        const m2 = Array.from(document.getElementById('team-2-members-list').querySelectorAll('input')).map(i=>i.value).filter(v=>v);
        
        gameSettings.team1Members = m1.length ? m1 : ['عضو 1'];
        gameSettings.team2Members = m2.length ? m2 : ['عضو 1'];
    }
...
function startNewRound() {
    ...
    currentMemberIndex = { red: 0, purple: 0 }; // تصفير الدور
    updatePlayerTurnDisplay(); // تحديث اسم اللاعب الحالي
    updateSidebars();
...
function updatePlayerTurnDisplay() {
    const redMem = gameSettings.team1Members[currentMemberIndex.red % gameSettings.team1Members.length];
    const purMem = gameSettings.team2Members[currentMemberIndex.purple % gameSettings.team2Members.length];
    
    document.getElementById('red-current-player').textContent = redMem;
    document.getElementById('purple-current-player').textContent = purMem;
}

function updateSidebars() {
    const r = document.getElementById('panel-red');
    const p = document.getElementById('panel-purple');
    r.classList.remove('active-turn-red'); p.classList.remove('active-turn-purple');
    if(gameSettings.mode==='competitive') return;
    if(currentTurn==='red') r.classList.add('active-turn-red');
    else p.classList.add('active-turn-purple');
}

function initializeBoard() {
    const c = document.getElementById('game-board-container'); c.innerHTML='';
    const s = [...ALL_LETTERS].sort(()=>0.5-Math.random());
    let idx=0;
    BOARD_LAYOUT.forEach((row, r) => {
        const d=document.createElement('div'); d.className='hex-row';
        row.forEach((t, col) => {
            const el=document.createElement('div'); el.className='hex-cell'; el.dataset.r=r; el.dataset.c=col;
            if(t==='G') {
                el.classList.add('hex-cell-default','playable');
                if(idx<25) { const l=s[idx++]; el.dataset.id=l.id; el.dataset.name=l.name; el.innerHTML=`<span class="hex-letter">${l.char}</span>`; }
                el.addEventListener('click', handleCellClick);
            } else if(t==='red') el.classList.add('hex-cell-red');
            else if(t==='purple') el.classList.add('hex-cell-purple');
            else el.classList.add('hex-cell-transparent');
            d.appendChild(el);
        });
        c.appendChild(d);
    });
...
function handleResult(res) {
    ...
    // معالجة الفائز
    if(res==='red') { winColor='red'; nextTurn=false; }
    else if(res==='purple') { winColor='purple'; nextTurn=false; }
    else if(res==='turn_correct') { winColor=currentTurn; nextTurn=true; }
    
    // في حالة الخطأ أو التخطي
    if(!winColor) {
        playSound('sound-click'); // صوت تخطي
        // تدوير الدور للفريق الآخر
        if(gameSettings.mode === 'turns') {
            // قبل قلب الدور، نزيد عداد العضو للفريق الحالي لأنه أنهى دوره
        ...
        // تدوير الدور
        if(nextTurn || gameSettings.mode==='turns') {
            currentMemberIndex[currentTurn]++; // العضو الحالي أنهى دوره
            currentTurn = (currentTurn==='red')?'purple':'red';
        }
    }

    updatePlayerTurnDisplay(); // تحديث اسم العضو الذي عليه الدور الآن
    updateSidebars();
}
...
function checkWin(color) {
    ...
    while(q.length>0) {
        ...
        for(const [dr,dc] of diffs) {
            ...
            if(win) {
                let k=`${r},${c}`; while(k){ const [pr,pc]=k.split(','); getC(pr,pc).classList.add('winning-path-cell'); k=parent.get(k); }
                return true;
            }
...
function handleGameWin(color) {
    ...
    startConfetti(); document.getElementById('confetti-canvas').style.display='block';
...
function startTimer(dur) {
    ...
    let rem = dur; const d=document.getElementById('question-timer'); d.classList.remove('hidden'); d.textContent=rem;
    timerInterval=setInterval(()=>{ rem--; d.textContent=rem; if(rem<=0){ clearInterval(timerInterval); handleResult('skip'); } },1000);
}
...
function startConfetti() {
    ...
    const p=[]; const col=['#f1c40f','#e74c3c','#8e44ad'];
    ...
    function d(){ ctx.clearRect(0,0,c.width,c.height); p.forEach(a=>{ a.y+=a.v; a.r+=2; if(a.y>c.height) a.y=-10; ctx.save(); ctx.translate(a.x,a.y); ctx.rotate(a.r*Math.PI/180); ctx.fillStyle=a.c; ctx.fillRect(-5*a.s,-5*a.s,10*a.s,10*a.s); ctx.restore(); }); requestAnimationFrame(d); }
    d();
}
